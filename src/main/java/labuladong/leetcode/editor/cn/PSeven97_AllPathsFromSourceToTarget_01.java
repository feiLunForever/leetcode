 ...
 *
 * @author DY
 * @date 2023-11-09 10:56:18
 */
public class PSeven97_AllPathsFromSourceToTarget_01 {
    public static void main(String[] args) {
        //测试代码
        Solution solution = new PSeven97_AllPathsFromSourceToTarget_01().new Solution();
    }
...
//leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public List<List<Integer>> allPathsSourceTarget(int[][] graph) {
            ...
            traverse(graph, 0, onPath, res);
            return res;
        }

        private void traverse(int[][] graph, int x, LinkedList<Integer> onPath, List<List<Integer>> res) {
            onPath.addLast(x);
            if (x == graph.length - 1) {
                res.add(new LinkedList<>(onPath));
            }
            for (int v : graph[x]) {
                traverse(graph, v, onPath, res);
            }
            onPath.removeLast();
        }
    }
