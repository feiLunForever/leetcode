 ...
 *
 * @author DY
 * @date 2023-08-03 15:47:36
 */
public class POne011_CapacityToShipPackagesWithinDDays_01 {
    public static void main(String[] args) {
        //测试代码
        Solution solution = new POne011_CapacityToShipPackagesWithinDDays_01().new Solution();
    }
    ...
    class Solution {
        public int shipWithinDays(int[] weights, int days) {
            ...
            while (left <= right) {
                ...
                int cost = calculate(weights, mid);
                if (cost > days) { // 花费的时间太长了，提高速度
                    left = mid + 1;
                } else if (cost < days) { // 花费的时间太少了，降到速度
                    right = mid - 1;
                } else { // 寻找左边界，缩短又边界
                    right = mid - 1;
                }
        ...
        private int calculate(int[] weights, int x) {
            int cost = 0;
